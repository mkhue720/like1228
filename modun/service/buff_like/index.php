...
if ($_REQUEST) {
    ...
    if ($type === 'buff_like') {
        ...
        $buff_like_reaction           = htmlspecialchars(addslashes($_POST['buff_like_reaction']));
        ...
        $check_name_facebook          = json_decode(file_get_contents('https://graph.facebook.com/' . $buff_like_facebook . '/?access_token=' . $settings_token), true);
        $buff_like_facebook_name      = $check_name_facebook['from']['name'];
        $buff_like_origin             = $check_like_facebook['summary']['total_count'];
        ...
        if ($total_payment > $user_point) {
            ...
            die(json_encode($return));
        } else if (!$check_name_facebook['id']) {
            $return['error'] = 1;
            $return['msg']   = 'ID Bài viết này không hợp lệ.';
            die(json_encode($return));
            ...
            VALUES ('$user_username', 'Mua tăng like bài viết.', '$total_payment', '$user_point', '$history_buy_ip', '$buff_like_time')");
            ...
            VALUES ('$user_username', '$buff_like_facebook_name', '$buff_like_facebook', '$buff_like_service', '$buff_like_content', '$buff_like_reaction', '$buff_like_amount', 'NO', '$buff_like_type', '$buff_like_gender', '$buff_like_origin', '0', '$buff_like_present', '$total_payment', 'progress', '$buff_like_time')")) {
