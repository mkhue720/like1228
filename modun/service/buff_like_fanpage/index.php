...
if ($_REQUEST) {
    ...
    if ($type === 'buff_like_fanpage') {
        ...
        $check_info_facebook          = json_decode(file_get_contents('https://graph.facebook.com/v5.0/' . $buff_like_facebook . '/?fields=name,fan_count&access_token=' . $settings_token), true);
        $buff_like_facebook_name      = $check_info_facebook['name'];
        $buff_like_origin             = $check_info_facebook['fan_count'];
        $buff_like_present	          = $check_info_facebook['fan_count'];
        ...
        if ($total_payment > $user_point) {
            ...
            $return['error'] = 1;
            $return['msg']   = 'Vui lòng mua tối thiểu ' . $check_service['service_min_amount'] . ' Follows.';
            die(json_encode($return));
            ...
            VALUES ('$user_username', 'Mua tăng like fanpage.', '$total_payment', '$user_point', '$history_buy_ip', '$buff_like_time')");
            ...
            VALUES ('$user_username', '$buff_like_facebook_name', '$buff_like_facebook', '$buff_like_service', '$buff_like_content', 'NO', '$buff_like_amount', 'NO', '$buff_like_type', '$buff_like_gender', '$buff_like_origin', '0', '$buff_like_present', '$total_payment', 'progress', '$buff_like_time')")) {
                $return['msg'] = 'Thêm ID tăng like fanpage thành công.';
                die(json_encode($return));
            } else {
                $return['error'] = 1;
