...
$result = mysqli_query($conn, "SELECT * FROM table_buff_like WHERE buff_like_service = 'buff-follows-facebook' AND buff_like_status = 'running'  ORDER BY RAND() LIMIT 10");
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $buff_like_increased    = $check_follows_facebook['summary']['total_count'];
    ...
    if (!$check_info_facebook['id']) {
        ...
        $return['error'] = 1;
        $return['msg']   = 'Id này đã chạy đủ theo dõi.';
        die(json_encode($return));
    } else {
        if (mysqli_query($conn, "UPDATE table_buff_like SET buff_like_increased = '$buff_like_increased', buff_like_present = '$buff_like_present' WHERE buff_like_facebook = '$buff_like_facebook'")) {
            $return['msg'] = 'Chạy api check buff theo dõi thành công.';
            die(json_encode($return));
        } else {
            $return['error'] = 1;
